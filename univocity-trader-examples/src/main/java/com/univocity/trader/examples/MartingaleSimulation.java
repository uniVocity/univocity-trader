...
//				.password("qwerty");
		...
		account.referenceCurrency("USDT") // Balances will be calculated using the reference currency.
				.tradeWith("BTC", "ETH").minimumInvestmentAmountPerTrade(10.0).maximumInvestmentAmountPerTrade(100)
...
//				.maximumInvestmentAmountPerAsset(200, "XRP")
		...
		account.strategies().add(AlwaysBuy::new);

		account.monitors().add(MartingaleStrategyMonitor::new);
		...
		simulation.initialFunds(1000.0).tradingFees(SimpleTradingFees.percentage(0.1)).emulateSlippage()
