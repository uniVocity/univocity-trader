...
//		If you want to receive e-mail notifications each time an order is submitted to the exchange, configure your e-mail sender
		trader.configure().mailSender().replyToAddress("dev@univocity.com").smtpHost("smtp.gmail.com").smtpSSL(true)
				.smtpPort(587).smtpUsername("<YOU>@gmail.com").smtpPassword("<YOUR SMTP PASSWORD>")
				.smtpSender("<YOU>>@gmail.com");

		// set an e-mail and timezone here to get notifications to your e-mail every
		// time a BUY or SELL happens.
		// the timezone is required if you want to host this in a server outside of your
		// local timezone
		// so the time a trade happens will come to you in your local time and not the
		// server time
		Account account = trader.configure().account().email("<YOUR E-MAIL").timeZone("system")
				.referenceCurrency("USDT").apiKey("<YOUR BINANCE API KEY>").secret("<YOUR BINANCE API SECRET>");
...
//		never invest more than 20 USDT on anything
		account.tradeWith("BTC", "ETH", "XRP", "ADA").maximumInvestmentAmountPerAsset(20);
