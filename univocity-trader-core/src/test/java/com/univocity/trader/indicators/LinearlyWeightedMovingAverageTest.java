...
public class LinearlyWeightedMovingAverageTest {
    ...
    public void lwma() {
        ...
        LinearlyWeightedMovingAverage indicator = new LinearlyWeightedMovingAverage(5, TimeInterval.MINUTE, c -> c.close);

        indicator.accumulate(CandleHelper.newCandle(i, values[i++]));
        assertEquals(37.08, indicator.getValue(), 0.001);

        indicator.accumulate(CandleHelper.newCandle(i, values[i++]));
        assertEquals(36.826, indicator.getValue(), 0.001);

        indicator.accumulate(CandleHelper.newCandle(i, values[i++]));
        assertEquals(36.468, indicator.getValue(), 0.001);

        indicator.accumulate(CandleHelper.newCandle(i, values[i++]));
        assertEquals(36.221, indicator.getValue(), 0.001);
