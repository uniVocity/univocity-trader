...
import static com.univocity.trader.candles.Candle.getFormattedDateTimeWithYear;
...
public class CandleRepository {
   ...
   private static DataSource dataSource = ThreadLocalDataSourceFactory.getInstance().getDataSource();
   ...
   private static Enumeration<Candle> executeQuery(String symbol, String query, Collection<Candle> out) {
      ...
      Runnable readingProcess = () -> {
         ...
         int count = 0;
         try (Connection c = db().getDataSource().getConnection(); PreparedStatement s = c.prepareStatement(query); ResultSet rs = s.executeQuery()) {
            while (rs.next()) {
