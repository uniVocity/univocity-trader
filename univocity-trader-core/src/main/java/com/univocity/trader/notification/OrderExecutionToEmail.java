...
import static com.univocity.trader.account.Balance.ZERO;
         ...
         if (assets > 0.0) {
            ...
            if (worth > 0.5) {
               ...
               if (trader.getTicks() > 0) {
                  msg.append(" (max: ").append(f.priceToString(trader.getMaxPrice())).append(", min: ").append(f.priceToString(trader.getMinPrice())).append(", length: ")
                        .append(trader.getFormattedTradeLength()).append(")");
               }
            }
         }
         if (locked > 0.0) {
            ...
            msg.append(f.quantityToString(locked)).append(" ").append(next.getAssetSymbol()).append(" locked in order (worth ~")
                  .append(f.switchToSymbol(next.getAssetSymbol() + next.getReferenceCurrencySymbol()).priceToString(locked * lastPrice)).append(" ").append(next.getFundSymbol()).append(')');
         }
      }
      if (visited.size() <= 1) {
