...
public class IQFeedProcessor {
	...
	public List<IQFeedCandle> processHistoricalResponse(String payload) {
		/**
		 * Result Format for HTX, HTD, and HTT requests: Request: HTX,GOOG,10<CR><LF>
		 * 2019-04-09
		 * 15:05:25.465607,1196.1200,1,513790,1196.1200,1196.3600,971,O,26,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:24.656735,1196.1200,15,513789,1196.1100,1196.3800,6148,O,5,113565084,3D87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:21.268339,1196.1200,1,513774,1196.1100,1196.4300,6147,O,5,113565084,3D87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.113481,1196.1200,1,513773,1196.1100,1196.4400,970,O,26,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.063681,1196.1200,8,513772,1196.1100,1196.4400,6146,O,5,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.063678,1196.1200,2,513764,1196.1100,1196.4400,6145,O,5,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.059843,1196.1200,98,513762,1196.1100,1196.4400,6144,O,5,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.059839,1196.1500,2,513664,1196.1100,1196.4400,6143,O,5,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.059381,1196.1200,2,513662,1196.1100,1196.4400,969,O,26,113565084,87,0,9,<CR><LF>
		 * 2019-04-09
		 * 15:05:20.059218,1196.1500,8,513660,1196.1100,1196.4400,6142,O,5,113565084,3D87,0,9,<CR><LF>
		 * !ENDMSG!,<CR><LF>
		 *
		 * Result Format for HIX, HID, and HIT requests: Request: HIT,GOOG,60,20130809
		 * 155500,20130812 093059,,,,1,TESTREQUEST,2,t<CR><LF> TESTREQUEST,2013-08-09
		 * 15:55:08,890.4600,890.3000,890.3500,890.4000,1220823,8289,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 15:56:06,890.4000,890.1100,890.3900,890.2700,1227889,7066,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 15:57:18,890.3700,890.0400,890.3700,890.2800,1236055,8166,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 15:58:14,890.4000,890.2000,890.2000,890.2200,1244583,8228,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 15:59:05,890.4300,890.2200,890.2200,890.3700,1251876,7293,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 15:59:48,890.4300,890.2000,890.3700,890.2000,1260653,8777,60,<CR><LF>
		 * TESTREQUEST,2013-08-09
		 * 18:24:47,892.0663,889.8600,890.2000,890.2300,1323761,63108,56,<CR><LF>
		 * TESTREQUEST,2013-08-12
		 * 09:30:11,888.0000,884.0100,884.0100,886.7200,24083,23958,60,<CR><LF>
		 * TESTREQUEST,2013-08-12
		 * 09:30:49,886.7980,886.0000,886.7980,886.4600,31219,7136,60,<CR><LF>
		 * TESTREQUEST,!ENDMSG!,<CR><LF>
		 *
		 * Result Format for HDX, HDD, HWT, HMX requests: Request:
		 * HWX,GOOG,10,1,TESTREQUEST,2,0<CR><LF>
	...
	public List<IQFeedCandle> processHTX(String payload) {
		...
		for (String line : payload.split("\n")) {
			...
			IQFeedCandle candle = new IQFeedCandle.IQFeedCandleBuilder().setOpenTime(openTime).setCloseTime(closeTime)
					.setLast(last).setLastSize(lastSize).setTotalVolume(totalVolume).setBid(bid).setAsk(ask)
					.setBasis(basis).setMarketCenter(marketCenter).setConditions(conditions).setAggressor(aggressor)
					.setDayCode(daycode).build();
			candles.add(candle);
		}
		return candles;
	}
	...
	public List<IQFeedCandle> processHIX(String payload) {
		...
		for (String line : payload.split("\n")) {
			...
			IQFeedCandle candle = new IQFeedCandle.IQFeedCandleBuilder().setOpenTime(openTime).setCloseTime(closeTime)
					.setOpen(open).setHigh(high).setLow(low).setClose(close).setVolume(periodVolume).build();
			candles.add(candle);
		}
		return candles;
	}
	...
	public List<IQFeedCandle> processHDX(String payload) {
		...
		for (String line : payload.split("\n")) {
			...
			IQFeedCandle candle = new IQFeedCandle.IQFeedCandleBuilder().setOpenTime(openTime).setCloseTime(closeTime)
					.setHigh(high).setLow(low).setOpen(open).setClose(close).setVolume(periodVolume)
					.setPeriodVolume(periodVolume).setOpenInterest(openInterest).build();
			candles.add(candle);
		}
		return candles;
	}
	...
	public Object processMarketSummaryResponse(String payload) {
		 ...
		 * Symbol,Exchange,Type,Last,TradeSize,TradedMarket,… AA$Y,34,11,1.64,,34,…..
		 * CBX$Y,34,11,1777.50,0,34,…… CZ$Y,34,11,1.92,,34,,…..
		 * NM$Y,34,11,20.0000,,34,…… NX$Y,34,11,16.7000,1,34,….. !ENDMSG!,<CR><LF>
		 */
		return null;
	}
	...
	public Object processNewsResponse(String payload) {
		/**
		 * examples are not properly provided for news response, will need to
