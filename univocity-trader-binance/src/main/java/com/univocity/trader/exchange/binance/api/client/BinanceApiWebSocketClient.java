...
/**
 * Binance API data streaming façade, supporting streaming of events through web
 * sockets.
 */
public interface BinanceApiWebSocketClient extends Closeable {

	/**
	 * Open a new web socket to receive {@link DepthEvent depthEvents} on a
	 * callback.
	 *
	 * @param symbols  market (one or coma-separated) symbol(s) to subscribe to
	 * @param callback the callback to call on new events
	 * @return a {@link Closeable} that allows the underlying web socket to be
	 *         closed.
	 */
	WebSocket onDepthEvent(String symbols, BinanceApiCallback<DepthEvent> callback);

	/**
	 * Open a new web socket to receive {@link CandlestickEvent candlestickEvents}
	 * on a callback.
	 *
	 * @param symbols  market (one or coma-separated) symbol(s) to subscribe to
	...
	/**
	 * Open a new web socket to receive {@link AggTradeEvent aggTradeEvents} on a
	 * callback.
	 *
	 * @param symbols  market (one or coma-separated) symbol(s) to subscribe to
	 * @param callback the callback to call on new events
	...
	/**
	 * Open a new web socket to receive {@link UserDataUpdateEvent
	 * userDataUpdateEvents} on a callback.
	 *
	...
	/**
	 * Open a new web socket to receive {@link AllMarketTickersEvent
	 * allMarketTickersEvents} on a callback.
	 *
	 * @param callback the callback to call on new events
	 * @return a {@link Closeable} that allows the underlying web socket to be
	 *         closed.
